...
ACMD(do_oasis_oedit)
{
    ...
    if(save)
    {
        ...
        mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "OLC: %s saves object info for zone %d.", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
...
static void oedit_disp_prompt_apply_menu(struct descriptor_data *d)
{
    ...
    for(counter = 0; counter < MAX_OBJ_AFFECT; counter++)
    {
        if(OLC_OBJ(d)->affected[counter].modifier)
        {
            ...
            write_to_output(d, " %s%d%s) %+d to %s\r\n", grn, counter + 1, nrm, OLC_OBJ(d)->affected[counter].modifier, apply_buf);
        }
        else
        {
...
static void oedit_disp_weapon_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter, nrm, attack_hit_text[counter].singular, !(++columns % 2) ? "\r\n" : "");
    }
    write_to_output(d, "\r\nEnter weapon type : ");
}
...
static void oedit_disp_spells_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %s%-20.20s %s", grn, counter, nrm, yel, spell_info[counter].name, !(++columns % 3) ? "\r\n" : "");
    }
...
static void oedit_disp_type_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter, nrm, item_types[counter], !(++columns % 2) ? "\r\n" : "");
    }
    write_to_output(d, "\r\nEnter object type : ");
}
...
static void oedit_disp_extra_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm, extra_bits[counter], !(++columns % 2) ? "\r\n" : "");
    }
...
static void oedit_disp_wear_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm, wear_bits[counter], !(++columns % 2) ? "\r\n" : "");
    }
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
    ...
    switch(OLC_MODE(d))
    {
        ...
        case OEDIT_TYPE:
            ...
            GET_OBJ_VAL(OLC_OBJ(d), 0) = GET_OBJ_VAL(OLC_OBJ(d), 1) = GET_OBJ_VAL(OLC_OBJ(d), 2) = GET_OBJ_VAL(OLC_OBJ(d), 3) = 0;
            break;
