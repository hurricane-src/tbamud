...
ACMD(do_oasis_medit)
{
    ...
    if(save)
    {
        ...
        mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "OLC: %s saves mobile info for zone %d.", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
...
static void medit_disp_stats_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    count_color_chars(buf) + 28,
                    buf,
                    yel,
                    GET_NDD(mob) + GET_DAMROLL(mob),
                    nrm,
                    yel,
