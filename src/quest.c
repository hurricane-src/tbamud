 ...
 *--------------------------------------------------------------------------*/
const char *quest_types[] = {"Object", "Room", "Find mob", "Kill mob", "Save mob", "Return object", "Clear room", "\n"};
...
void parse_quest(FILE *quest_f, int nr)
{
    ...
    if(!get_line(quest_f, line) || (retval = sscanf(line, " %d %d %s %d %d %d %d", t, t + 1, f1, t + 2, t + 3, t + 4, t + 5)) != 7)
    {
    ...
    if(!get_line(quest_f, line) || (retval = sscanf(line, " %d %d %d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4, t + 5, t + 6)) != 7)
    {
        log("Format error in numeric line (expected 7, got %d), %s\n", retval, line);
        exit(1);
    }
...
void list_quests(struct char_data *ch, zone_rnum zone, qst_vnum vmin, qst_vnum vmax)
{
    ...
    for(rnum = 0; rnum < total_quests; rnum++)
    {
        ...
        {
            send_to_char(ch, "\tg%4d\tn) [\tg%-5d\tn] \tc%-44.44s\tn \ty[%5d]\tn\r\n", ++counter, QST_NUM(rnum), QST_NAME(rnum), QST_MASTER(rnum) == NOBODY ? 0 : QST_MASTER(rnum));
        }
    }
    if(!counter)
    {
...
static void quest_hist(struct char_data *ch)
{
    ...
    send_to_char(ch,
                 "Quests that you have completed:\r\n"
...
static void quest_join(struct char_data *ch, struct char_data *qm, char argument[MAX_INPUT_LENGTH])
{
    ...
    else if((QST_PREREQ(rnum) != NOTHING) && (real_object(QST_PREREQ(rnum)) != NOTHING) && (get_obj_in_list_num(real_object(QST_PREREQ(rnum)), ch->carrying) == NULL))
    {
        snprintf(buf, sizeof(buf), "%s You need to have %s first!", GET_NAME(ch), obj_proto[real_object(QST_PREREQ(rnum))].short_description);
    }
    else
    {
        ...
        if(QST_TIME(rnum) != -1)
        {
            snprintf(buf, sizeof(buf), "%s You have a time limit of %d turn%s to complete the quest.", GET_NAME(ch), QST_TIME(rnum), QST_TIME(rnum) == 1 ? "" : "s");
        }
        else
        {
...
static void quest_show(struct char_data *ch, mob_vnum qm)
{
    ...
    for(rnum = 0; rnum < total_quests; rnum++)
    {
        if(qm == QST_MASTER(rnum))
        {
            send_to_char(ch, "\tg%4d\tn) \tc%-52.52s\tn \ty(%5d)\tn \ty(%s)\tn\r\n", ++counter, QST_DESC(rnum), QST_NUM(rnum), (is_complete(ch, QST_NUM(rnum)) ? "Yes" : "No "));
        }
    }
    if(!counter)
    {
...
static void quest_stat(struct char_data *ch, char argument[MAX_STRING_LENGTH])
{
    ...
    else
    {
        ...
        switch(QST_TYPE(rnum))
        {
            case AQ_OBJ_FIND:
            case AQ_OBJ_RETURN:
                snprintf(targetname, sizeof(targetname), "%s", real_object(QST_TARGET(rnum)) == NOTHING ? "An unknown object" : obj_proto[real_object(QST_TARGET(rnum))].short_description);
                break;
            case AQ_ROOM_FIND:
            case AQ_ROOM_CLEAR:
                snprintf(targetname, sizeof(targetname), "%s", real_room(QST_TARGET(rnum)) == NOWHERE ? "An unknown room" : world[real_room(QST_TARGET(rnum))].name);
                break;
            case AQ_MOB_FIND:
            case AQ_MOB_KILL:
            case AQ_MOB_SAVE:
                snprintf(targetname, sizeof(targetname), "%s", real_mobile(QST_TARGET(rnum)) == NOBODY ? "An unknown mobile" : GET_NAME(&mob_proto[real_mobile(QST_TARGET(rnum))]));
                break;
            default:
        ...
        {
            send_to_char(ch, "Mob   : [\ty%5d\tn] \ty%s\tn\r\n", QST_RETURNMOB(rnum), real_mobile(QST_RETURNMOB(rnum)) == NOBODY ? "an unknown mob" : mob_proto[real_mobile(QST_RETURNMOB(rnum))].player.short_descr);
        }
        if(QST_TIME(rnum) != -1)
        {
