...
static void prefedit_disp_main_menu(struct descriptor_data *d)
{
    ...
    sprintf(prompt_string, "%s%s%s", PREFEDIT_FLAGGED(PRF_DISPHP) ? "H" : "", PREFEDIT_FLAGGED(PRF_DISPMANA) ? "M" : "", PREFEDIT_FLAGGED(PRF_DISPMOVE) ? "V" : "");

    sprintf(color_string, "%s", multi_types[(PREFEDIT_FLAGGED(PRF_COLOR_1) ? 1 : 0) + (PREFEDIT_FLAGGED(PRF_COLOR_2) ? 2 : 0)]);

    send_to_char(d->character, "\r\n%sPreferences for %s%s\r\n", CCYEL(d->character, C_NRM), GET_NAME(vict), CCNRM(d->character, C_NRM));
    ...
    if(GET_LEVEL(PREFEDIT_GET_CHAR) >= LVL_IMMORT)
    {
        ...
        send_to_char(d->character,
                     ...
                     CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_CLS)),
                     CCCYN(d->character, C_NRM),
                     ...
                     CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_SHOWVNUMS)),
                     CCCYN(d->character, C_NRM),
                     CBYEL(d->character, C_NRM),
                     CCNRM(d->character, C_NRM),
                     CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_NOWIZ)),
                     CCCYN(d->character, C_NRM),
                     ...
                     CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_NOHASSLE)),
                     CCCYN(d->character, C_NRM),
                     CBYEL(d->character, C_NRM),
                     CCNRM(d->character, C_NRM),
                     CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_HOLYLIGHT)),
                     CCCYN(d->character, C_NRM));
        if(GET_LEVEL(PREFEDIT_GET_CHAR) == LVL_IMPL)
        {
            send_to_char(d->character,
                         "%s7%s) Zone Resets  %s[%s%3s%s]\r\n",
                         CBYEL(d->character, C_NRM),
                         CCNRM(d->character, C_NRM),
                         CCCYN(d->character, C_NRM),
                         CCYEL(d->character, C_NRM),
                         ONOFF(PREFEDIT_FLAGGED(PRF_ZONERESETS)),
                         CCCYN(d->character, C_NRM));
        }
    }
...
static void prefedit_disp_toggles_menu(struct descriptor_data *d)
{
    ...
    send_to_char(d->character,
                 ...
                 ONOFF(PREFEDIT_FLAGGED(PRF_AUTOMAP)),
                 CCCYN(d->character, C_NRM),
                 /* Line 8 - autokey */
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 ...
                 ONOFF(PREFEDIT_FLAGGED(PRF_AUTOKEY)),
                 CCCYN(d->character, C_NRM),
                 /* Line 9 - autodoor */
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 ...
                 ONOFF(PREFEDIT_FLAGGED(PRF_AUTODOOR)),
                 CCCYN(d->character, C_NRM));
    ...
    send_to_char(d->character,
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(!PREFEDIT_FLAGGED(PRF_SUMMONABLE)),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(PREFEDIT_FLAGGED(PRF_BRIEF)),
                 CCCYN(d->character, C_NRM),
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(PREFEDIT_FLAGGED(PRF_NOREPEAT)),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(PREFEDIT_FLAGGED(PRF_COMPACT)),
                 CCCYN(d->character, C_NRM));
    ...
    send_to_char(d->character,
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->pVariables[eMSDP_XTERM_256_COLORS]->ValueInt),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->pVariables[eMSDP_MXP]->ValueInt),
                 CCCYN(d->character, C_NRM),
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->pVariables[eMSDP_ANSI_COLORS]->ValueInt),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->bMSDP),
                 CCCYN(d->character, C_NRM),
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->bCHARSET),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->bATCP),
                 CCCYN(d->character, C_NRM),
                 ...
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->pVariables[eMSDP_UTF_8]->ValueInt),
                 CCCYN(d->character, C_NRM),
                 CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM),
                 CCCYN(d->character, C_NRM),
                 CCYEL(d->character, C_NRM),
                 ONOFF(d->pProtocol->bMSP),
                 CCCYN(d->character, C_NRM));
    /* Finishing Off */
