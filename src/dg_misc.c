...
void do_dg_cast(void *go, struct script_data *sc, trig_data *trig, int type, char *cmd)
{
    ...
    {
        script_log("Trigger: %s, VNum %d. dg_cast: invalid spell name (%s)", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), orig_cmd);
        return;
    }
...
void script_damage(struct char_data *vict, int dam)
{
    ...
    {
        send_to_char(vict,
                     "Being the cool immortal you are, you sidestep a trap, "
    ...
    if(GET_POS(vict) == POS_DEAD)
    {
        if(!IS_NPC(vict))
        {
            mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(vict)), TRUE, "%s killed by script at %s", GET_NAME(vict), vict->in_room == NOWHERE ? "NOWHERE" : world[vict->in_room].name);
        }
        die(vict, NULL);
    }
}
