...
static int export_save_shops(zone_rnum zrnum)
{
    ...
    for(i = genolc_zone_bottom(zrnum); i <= zone_table[zrnum].top; i++)
    {
        if((rshop = real_shop(i)) != NOWHERE)
        {
            ...
            {
                if(obj_index[S_PRODUCT(shop, j)].vnum < genolc_zone_bottom(zrnum) || obj_index[S_PRODUCT(shop, j)].vnum > zone_table[zrnum].top)
                {
                    continue;
                }
    ...
    for(subcmd = 0; ZCMD(zrnum, subcmd).command != 'S'; subcmd++)
    {
        switch(ZCMD(zrnum, subcmd).command)
        {
            ...
            case 'O':
                ...
                break;
            case 'G':
                fprintf(zone_file, "G %d QQ%02d %d -1 \t(%s)\n", ZCMD(zrnum, subcmd).if_flag, obj_index[ZCMD(zrnum, subcmd).arg1].vnum % 100, ZCMD(zrnum, subcmd).arg2, obj_proto[ZCMD(zrnum, subcmd).arg1].short_description);
                break;
            case 'E':
                fprintf(zone_file,
                        "E %d QQ%02d %d %d \t(%s)\n",
            ...
            case 'P':
                ...
                break;
            case 'D':
                fprintf(zone_file, "D %d QQ%02d %d %d \t(%s)\n", ZCMD(zrnum, subcmd).if_flag, world[ZCMD(zrnum, subcmd).arg1].number % 100, ZCMD(zrnum, subcmd).arg2, ZCMD(zrnum, subcmd).arg3, world[ZCMD(zrnum, subcmd).arg1].name);
                break;
            case 'R':
                fprintf(zone_file,
                        "R %d QQ%02d QQ%02d -1 \t(%s)\n",
                        ZCMD(zrnum, subcmd).if_flag,
...
static int export_save_objects(zone_rnum zrnum)
{
    ...
    for(ovnum = genolc_zone_bottom(zrnum); ovnum <= zone_table[zrnum].top; ovnum++)
    {
        if((ornum = real_object(ovnum)) != NOTHING)
        {
            ...
            fprintf(obj_file, "%d %s %s %s %s %s %s %s %s %s %s %s %s\n", GET_OBJ_TYPE(obj), ebuf1, ebuf2, ebuf3, ebuf4, wbuf1, wbuf2, wbuf3, wbuf4, pbuf1, pbuf2, pbuf3, pbuf4);
