...
void save_player_index(void)
{
    ...
    for(i = 0; i <= top_of_p_table; i++)
    {
        if(*player_table[i].name)
        {
            ...
            fprintf(index_file, "%ld %s %d %s %ld\n", player_table[i].id, player_table[i].name, player_table[i].level, *bits ? bits : "0", (long)player_table[i].last);
        }
    }
    fprintf(index_file, "~\n");
...
void save_char(struct char_data *ch)
{
    ...
    if(tmp_aff[0].spell > 0)
    {
        ...
        for(i = 0; i < MAX_AFFECT; i++)
        {
            ...
            if(aff->spell)
            {
                fprintf(fl, "%d %d %d %d %d %d %d %d\n", aff->spell, aff->duration, aff->modifier, aff->location, aff->bitvector[0], aff->bitvector[1], aff->bitvector[2], aff->bitvector[3]);
            }
        }
    ...
    for(i = 0; i <= top_of_p_table; i++)
    {
        ...
        if(!IS_SET(player_table[i].flags, PINDEX_NODELETE) && *player_table[i].name)
        {
            ...
            else
            {
                ...
                {
                    if(player_table[i].level <= pclean_criteria[ci].level && ((time(0) - player_table[i].last) > (pclean_criteria[ci].days * SECS_PER_REAL_DAY)))
                    {
                        remove_player(i);
                        break;
                    }
