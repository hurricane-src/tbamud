...
int save_quests(zone_rnum zone_num)
{
    ...
    log("GenOLC: save_quests: Saving quests in zone #%d (%d-%d).", zone_table[zone_num].number, genolc_zone_bottom(zone_num), zone_table[zone_num].top);
    ...
    for(i = genolc_zone_bottom(zone_num); i <= zone_table[zone_num].top; i++)
    {
        ...
        if((rnum = real_quest(i)) != NOTHING)
        {
            ...
            int n = snprintf(buf,
                             ...
                             "S\n",
                             QST_NUM(rnum),
                             QST_NAME(rnum) ? QST_NAME(rnum) : "Untitled",
                             STRING_TERMINATOR,
                             quest_desc,
                             STRING_TERMINATOR,
                             quest_info,
            ...
            {
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Could not save quest #%d due to size (%d > maximum of %d).", QST_NUM(rnum), n, MAX_STRING_LENGTH);
            }
        }
    }
