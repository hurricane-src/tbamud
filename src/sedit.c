...
ACMD(do_oasis_sedit)
{
    ...
    if(save)
    {
        ...
        mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "OLC: %s saves shop info for zone %d.", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
...
static void sedit_products_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%2d - [%s%5d%s] - %s%s%s\r\n", i, cyn, obj_index[S_PRODUCT(shop, i)].vnum, nrm, yel, obj_proto[S_PRODUCT(shop, i)].short_description, nrm);
    }
    write_to_output(d,
                    "\r\n"
...
static void sedit_types_menu(struct descriptor_data *d)
{
    ...
    {
        write_to_output(d, "%s%2d%s) %s%-20s%s  %s", grn, i, nrm, cyn, item_types[i], nrm, !(++count % 3) ? "\r\n" : "");
    }
        ...
        case SEDIT_CONFIRM_SAVESTRING:
            switch(*arg)
            {
                ...
                case 'Y':
                    ...
                    mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s edits shop %d", GET_NAME(d->character), OLC_NUM(d));
                    if(CONFIG_OLC_SAVE)
                    {
