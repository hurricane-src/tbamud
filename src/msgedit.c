...
ACMD(do_msgedit)
{
    ...
    mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "OLC: %s starts editing message %d", GET_NAME(ch), OLC_NUM(d));
}
...
static void msgedit_main_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    yel,
                    nrm,
                    PRINT_MSG(OLC_MSG(d)->god_msg.attacker_msg),
                    grn,
                    yel,
                    nrm,
                    PRINT_MSG(OLC_MSG(d)->god_msg.victim_msg),
                    grn,
                    yel,
                    nrm,
