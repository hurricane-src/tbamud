...
void dg_read_trigger(FILE *fp, void *proto, int type)
{
    ...
    if(rnum == NOTHING)
    {
        ...
        {
            case MOB_TRIGGER:
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (mob: %s - %d)", vnum, GET_NAME((char_data *)proto), GET_MOB_VNUM((char_data *)proto));
                break;
            case WLD_TRIGGER:
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (room:%d)", vnum, GET_ROOM_VNUM(((room_data *)proto)->number));
                break;
            default:
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (?)", vnum);
                break;
        }
        return;
    }
    ...
    switch(type)
    {
        ...
        case WLD_TRIGGER:
            ...
            {
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: non-existant trigger #%d assigned to room #%d", vnum, room->number);
            }
            break;
        default:
...
void dg_obj_trigger(char *line, struct obj_data *obj)
{
    ...
    if(rnum == NOTHING)
    {
        mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Trigger vnum #%d asked for but non-existant! (Object: %s - %d)", vnum, obj->short_description, GET_OBJ_VNUM(obj));
        return;
    }
    ...
    switch(type)
    {
        case MOB_TRIGGER:
            ...
            while(trg_proto)
            {
                ...
                if(rnum == NOTHING)
                {
                    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: trigger #%d non-existant, for mob #%d", trg_proto->vnum, mob_index[mob->nr].vnum);
                }
                else
                {
                    if(!SCRIPT(mob))
        ...
        case OBJ_TRIGGER:
            ...
            while(trg_proto)
            {
                ...
                if(rnum == NOTHING)
                {
                    log("SYSERR: trigger #%d non-existant, for obj #%d", trg_proto->vnum, obj_index[obj->item_number].vnum);
                }
                else
                {
                    if(!SCRIPT(obj))
        ...
        case WLD_TRIGGER:
            ...
            while(trg_proto)
            {
                ...
                if(rnum == NOTHING)
                {
                    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: trigger #%d non-existant, for room #%d", trg_proto->vnum, room->number);
                }
                else
                {
                    if(!SCRIPT(room))
