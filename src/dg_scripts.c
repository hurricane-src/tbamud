...
static struct cmdlist_element *find_else_end(trig_data *trig, struct cmdlist_element *cl, void *go, struct script_data *sc, int type);
...
static struct cmdlist_element *find_case(struct trig_data *trig, struct cmdlist_element *cl, void *go, struct script_data *sc, int type, char *cond);
...
int find_eq_pos_script(char *arg)
{
    ...
    struct eq_pos_list
    {
        const char *pos;
        int         where;
    } eq_pos[] = {{"hold", WEAR_HOLD},   {"held", WEAR_HOLD},      {"light", WEAR_LIGHT},    {"wield", WEAR_WIELD}, {"rfinger", WEAR_FINGER_R}, {"lfinger", WEAR_FINGER_L}, {"neck1", WEAR_NECK_1},  {"neck2", WEAR_NECK_2},
                  {"body", WEAR_BODY},   {"head", WEAR_HEAD},      {"legs", WEAR_LEGS},      {"feet", WEAR_FEET},   {"hands", WEAR_HANDS},      {"arms", WEAR_ARMS},        {"shield", WEAR_SHIELD}, {"about", WEAR_ABOUT},
                  {"waist", WEAR_WAIST}, {"rwrist", WEAR_WRIST_R}, {"lwrist", WEAR_WRIST_L}, {"none", -1}};
...
void check_time_triggers(void)
{
    ...
    for(ch = character_list; ch; ch = ch->next)
    {
        if(SCRIPT(ch))
        {
            ...
            if(IS_SET(SCRIPT_TYPES(sc), WTRIG_TIME) && (!is_empty(world[IN_ROOM(ch)].zone) || IS_SET(SCRIPT_TYPES(sc), WTRIG_GLOBAL)))
            {
                time_mtrigger(ch);
            }
        }
    ...
    for(nr = 0; nr <= top_of_world; nr++)
    {
        if(SCRIPT(&world[nr]))
        {
            ...
            if(IS_SET(SCRIPT_TYPES(sc), WTRIG_TIME) && (!is_empty(room->zone) || IS_SET(SCRIPT_TYPES(sc), WTRIG_GLOBAL)))
            {
                time_wtrigger(room);
            }
        }
...
static void do_stat_trigger(struct char_data *ch, trig_data *trig)
{
    ...
    len += snprintf(sb, sizeof(sb), "Name: '%s%s%s',  VNum: [%s%5d%s], RNum: [%5d]\r\n", CCYEL(ch, C_NRM), GET_TRIG_NAME(trig), CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_TRIG_VNUM(trig), CCNRM(ch, C_NRM), GET_TRIG_RNUM(trig));
...
static void script_stat(char_data *ch, struct script_data *sc)
{
    ...
    {
        send_to_char(ch, "\r\n  Trigger: %s%s%s, VNum: [%s%5d%s], RNum: [%5d]\r\n", CCYEL(ch, C_NRM), GET_TRIG_NAME(t), CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_TRIG_VNUM(t), CCNRM(ch, C_NRM), GET_TRIG_RNUM(t));
...
ACMD(do_attach)
{
    ...
    else if(is_abbrev(arg, "object") || is_abbrev(arg, "otr"))
    {
        ...
        send_to_char(ch, "Trigger %d (%s) attached to %s [%d].\r\n", tn, GET_TRIG_NAME(trig), (object->short_description ? object->short_description : object->name), GET_OBJ_VNUM(object));
    }
...
ACMD(do_detach)
{
    ...
    else
    {
        ...
        else if(object)
        {
            ...
            {
                send_to_char(ch, "Trigger %d (%s) removed from %s.\r\n", tn, GET_TRIG_NAME(trig), object->short_description ? object->short_description : object->name);
        ...
        else if(is_abbrev(arg, "obj"))
        {
            ...
            switch(type)
            {
                ...
                case OBJ_TRIGGER:
                    ...
                    break;
                case MOB_TRIGGER:
                    if((o = get_obj_in_list_vis((struct char_data *)go, name, NULL, ((struct char_data *)go)->carrying)) == NULL)
                    {
                        o = get_obj_in_list_vis((struct char_data *)go, name, NULL, world[IN_ROOM((struct char_data *)go)].contents);
                    }
                    break;
            }
            if(o)
    ...
    if(depth > MAX_SCRIPT_DEPTH)
    {
        ...
        switch(type)
        {
            case MOB_TRIGGER:
                ...
                break;
            case OBJ_TRIGGER:
                script_log("It was attached to %s [%d]", ((obj_data *)go)->short_description, GET_OBJ_VNUM((obj_data *)go));
                break;
            case WLD_TRIGGER:
...
void save_char_vars(struct char_data *ch)
{
    ...
    if(!file)
    {
        mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Could not open player variable file %s for writing.:%s", fn, strerror(errno));
        return;
    }
