...
void affect_total(struct char_data *ch)
{
    ...
    for(i = 0; i < NUM_WEARS; i++)
    {
        if(GET_EQ(ch, i))
        {
            ...
            {
                affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location, GET_EQ(ch, i)->affected[j].modifier, GET_OBJ_AFFECT(GET_EQ(ch, i)), FALSE);
            }
        }
    }
    ...
    for(i = 0; i < NUM_WEARS; i++)
    {
        if(GET_EQ(ch, i))
        {
            for(j = 0; j < MAX_OBJ_AFFECT; j++)
            {
                affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location, GET_EQ(ch, i)->affected[j].modifier, GET_OBJ_AFFECT(GET_EQ(ch, i)), TRUE);
            }
        }
    }
...
void obj_from_room(struct obj_data *object)
{
    ...
    {
        log("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room", (void *)object, IN_ROOM(object));
        return;
    }
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
    ...
    {
        log("SYSERR: NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.", (void *)obj, (void *)obj, (void *)obj_to);
        return;
    }
...
struct obj_data *create_money(int amount)
{
    ...
    else
    {
        ...
        {
            snprintf(buf, sizeof(buf), "You guess there are, maybe, %d coins.", 1000 * ((amount / 1000) + rand_number(0, (amount / 1000))));
        }
        else
        {
