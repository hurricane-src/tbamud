...
/* Shop purchase adjustment, based on charisma-difference from buyer to keeper.
   ...
   having a 3. */
static int buy_price(struct obj_data *obj, int shop_nr, struct char_data *keeper, struct char_data *buyer) { return (int)(GET_OBJ_COST(obj) * SHOP_BUYPROFIT(shop_nr) * (1 + (GET_CHA(keeper) - GET_CHA(buyer)) / (float)70)); }
...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if(OBJ_FLAGGED(obj, ITEM_QUEST))
    {
        while(obj && (GET_QUESTPOINTS(ch) >= GET_OBJ_COST(obj) || IS_GOD(ch)) && IS_CARRYING_N(ch) < CAN_CARRY_N(ch) && bought < buynum && IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch))
        {
            bought++;
...
static void list_all_shops(struct char_data *ch)
{
    ...
    *buf = '\0';
    for(shop_nr = 0; shop_nr <= top_shop && len < sizeof(buf); shop_nr++)
    {
        ...
        len += snprintf(
            buf + len, sizeof(buf) - len, "%3d   %6d   %6d    %s   %3.2f   %3.2f    %s\r\n", shop_nr + 1, SHOP_NUM(shop_nr), SHOP_ROOM(shop_nr, 0), buf1, SHOP_SELLPROFIT(shop_nr), SHOP_BUYPROFIT(shop_nr), customer_string(shop_nr, FALSE));
    }
...
static void list_detailed_shop(struct char_data *ch, int shop_nr)
{
    ...
    for(sindex = 0; SHOP_PRODUCT(shop_nr, sindex) != NOTHING; sindex++)
    {
        ...
        linelen = snprintf(buf1, sizeof(buf1), "%s (#%d)", obj_proto[SHOP_PRODUCT(shop_nr, sindex)].short_description, obj_index[SHOP_PRODUCT(shop_nr, sindex)].vnum);
    ...
    for(sindex = 0; SHOP_BUYTYPE(shop_nr, sindex) != NOTHING; sindex++)
    {
        ...
        linelen = snprintf(buf1, sizeof(buf1), "%s (#%d) [%s]", item_types[SHOP_BUYTYPE(shop_nr, sindex)], SHOP_BUYTYPE(shop_nr, sindex), SHOP_BUYWORD(shop_nr, sindex) ? SHOP_BUYWORD(shop_nr, sindex) : "all");
...
bool shopping_identify(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    send_to_char(ch, "Weight: %d, Cost to Sell: %s%d%s, Cost to Buy: %s%d%s\r\n", GET_OBJ_WEIGHT(obj), QYEL, sell_price(obj, shop_nr, keeper, ch), QNRM, QYEL, buy_price(obj, shop_nr, keeper, ch), QNRM);
    ...
    switch(GET_OBJ_TYPE(obj))
    {
        ...
        case ITEM_SCROLL:
        case ITEM_POTION:
            send_to_char(ch, "Spells: %s, %s, %s\r\n", skill_name(GET_OBJ_VAL(obj, 1)), skill_name(GET_OBJ_VAL(obj, 2)), skill_name(GET_OBJ_VAL(obj, 3)));
            break;
        case ITEM_WAND:
        case ITEM_STAFF:
