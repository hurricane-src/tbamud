        ...
        if(cmd_room == rrnum)
        {
            ...
            switch(ZOCMD.command)
            {
                ...
                case 'P':
                    ...
                    break;
                case 'R':
                    send_to_char(ch, "%sRemove %s [%s%d%s] from room.\r\n", ZOCMD.if_flag ? " then " : "", obj_proto[ZOCMD.arg2].short_description, cyn, obj_index[ZOCMD.arg2].vnum, yel);
                    break;
                case 'D':
                    send_to_char(ch, "%sSet door %s as %s.\r\n", ZOCMD.if_flag ? " then " : "", dirs[ZOCMD.arg2], ZOCMD.arg3 ? ((ZOCMD.arg3 == 1) ? "closed" : "locked") : "open");
                    break;
                case 'T':
                    send_to_char(ch,
                                 "%sAttach trigger %s%s%s [%s%d%s] to %s\r\n",
                                 ZOCMD.if_flag ? " then " : "",
                                 cyn,
...
static void do_stat_room(struct char_data *ch, struct room_data *rm)
{
    ...
    send_to_char(ch, "Zone: [%3d], VNum: [%s%5d%s], RNum: [%5d], IDNum: [%5ld], Type: %s\r\n", zone_table[rm->zone].number, CCGRN(ch, C_NRM), rm->number, CCNRM(ch, C_NRM), real_room(rm->number), room_script_id(rm), buf2);
...
static void do_stat_object(struct char_data *ch, struct obj_data *j)
{
    ...
    send_to_char(ch, "Name: '%s%s%s', Keywords: %s\r\n", CCYEL(ch, C_NRM), j->short_description ? j->short_description : "<None>", CCNRM(ch, C_NRM), j->name);

    vnum = GET_OBJ_VNUM(j);
    sprinttype(GET_OBJ_TYPE(j), item_types, buf, sizeof(buf));
    send_to_char(
        ch, "VNum: [%s%5d%s], RNum: [%5d], Idnum: [%5ld], Type: %s, SpecProc: %s\r\n", CCGRN(ch, C_NRM), vnum, CCNRM(ch, C_NRM), GET_OBJ_RNUM(j), obj_script_id(j), buf, GET_OBJ_SPEC(j) ? (get_spec_func_name(GET_OBJ_SPEC(j))) : "None");

    send_to_char(ch, "L-Desc: '%s%s%s'\r\n", CCYEL(ch, C_NRM), j->description ? j->description : "<None>", CCNRM(ch, C_NRM));

    send_to_char(ch, "A-Desc: '%s%s%s'\r\n", CCYEL(ch, C_NRM), j->action_description ? j->action_description : "<None>", CCNRM(ch, C_NRM));
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
    ...
    send_to_char(ch,
                 "%s %s '%s'  IDNum: [%5ld], In room [%5d], Loadroom : [%5d]\r\n",
                 buf,
    ...
    send_to_char(ch,
                 "%s%s, Lev: [%s%2d%s], XP: [%s%7d%s], Align: [%4d]\r\n",
    ...
    if(!IS_NPC(k))
    {
        ...
        send_to_char(ch, "Played: [%dh %dm], Age: [%d], STL[%d]/per[%d]/NSTL[%d]", k->player.time.played / 3600, (k->player.time.played % 3600) / 60, age(k)->year, GET_PRACTICES(k), int_app[GET_INT(k)].learn, wis_app[GET_WIS(k)].bonus);
    ...
    send_to_char(ch, "Gold: [%9d], Bank: [%9d] (Total: %d), ", GET_GOLD(k), GET_BANK_GOLD(k), GET_GOLD(k) + GET_BANK_GOLD(k));

    if(!IS_NPC(k))
    {
        send_to_char(ch, "Screen %s[%s%d%sx%s%d%s]%s\r\n", CCCYN(ch, C_NRM), CCYEL(ch, C_NRM), GET_SCREEN_WIDTH(k), CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), GET_PAGE_LENGTH(k), CCCYN(ch, C_NRM), CCNRM(ch, C_NRM));
    }
    ...
    if(IS_MOB(k))
    {
        send_to_char(ch, "Mob Spec-Proc: %s, NPC Bare Hand Dam: %dd%d\r\n", (mob_index[GET_MOB_RNUM(k)].func ? get_spec_func_name(mob_index[GET_MOB_RNUM(k)].func) : "None"), k->mob_specials.damnodice, k->mob_specials.damsizedice);
    }
...
static const char *last_array[11] = {"Connect", "Enter Game", "Reconnect", "Takeover", "Quit", "Idleout", "Disconnect", "Shutdown", "Reboot", "Crash", "Playing"};
...
static void mod_llog_entry(struct last_entry *llast, int type)
{
    ...
    for(tmp = recs; tmp > 0; tmp--)
    {
        ...
        if(mlast.idnum == llast->idnum && mlast.punique == llast->punique)
        {
            ...
            if(mlast.close_type != LAST_QUIT && mlast.close_type != LAST_IDLEOUT && mlast.close_type != LAST_REBOOT && mlast.close_type != LAST_SHUTDOWN)
            {
...
static void list_llog_entries(struct char_data *ch)
{
    ...
    while(fread(&llast, sizeof(struct last_entry), 1, fp) == 1)
    {
        ...
        send_to_char(ch, "%10s    %d    %s    %s\r\n", llast.username, llast.punique, last_array[llast.close_type], timestr);
        break;
    }
...
ACMD(do_last)
{
    ...
    *name = '\0';
    ...
    if(*name && !num)
    {
        ...
        send_to_char(ch, "[%5ld] [%2d %s] %-12s : %-18s : %-24s\r\n", GET_IDNUM(vict), (int)GET_LEVEL(vict), class_abbrevs[(int)GET_CLASS(vict)], GET_NAME(vict), GET_HOST(vict) && *GET_HOST(vict) ? GET_HOST(vict) : "(NOHOST)", timestr);
        free_char(vict);
        return;
    }
...
ACMD(do_wiznet)
{
    ...
    for(d = descriptor_list; d; d = d->next)
    {
        if(IS_PLAYING(d) && (GET_LEVEL(d->character) >= level) && (!PRF_FLAGGED(d->character, PRF_NOWIZ)) && (d != ch->desc || !(PRF_FLAGGED(d->character, PRF_NOREPEAT))))
        {
    ...
    else
    {
        switch(subcmd)
        {
            ...
            case SCMD_MUTE:
                ...
                mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) Mute %s for %s by %s.", ONOFF(result), GET_NAME(vict), GET_NAME(ch));
            ...
            case SCMD_THAW:
                ...
                break;
            case SCMD_UNAFFECT:
                /// @todo 20250321 Hurricane --  AFF_FLAGS(vict): I'm assuming it's checking the first 32 bits ...
                if(vict->affected || AFF_FLAGS(vict)[0])
                {
                    while(vict->affected)
                    {
...
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone, int listall)
{
    ...
    if(listall)
    {
        ...
        tmp += snprintf(bufptr + tmp,
                        ...
                        "         Quests:   %2d\r\n",
                        buf,
                        zone_table[zone].min_level,
                        zone_table[zone].max_level,
                        j,
                        k,
                        l,
                        m,
...
ACMD(do_show)
{
    ...
    switch(l)
    {
        ...
        case 2:
        {
            ...
            send_to_char(ch, "Player: %-12s (%s) [%2d %s]\r\n", GET_NAME(vict), genders[(int)GET_SEX(vict)], GET_LEVEL(vict), class_abbrevs[(int)GET_CLASS(vict)]);
            send_to_char(ch, "Gold: %-8d  Bal: %-8d Exp: %-8d  Align: %-5d  Lessons: %-3d\r\n", GET_GOLD(vict), GET_BANK_GOLD(vict), GET_EXP(vict), GET_ALIGNMENT(vict), GET_PRACTICES(vict));
        ...
        case 11:
            ...
            {
                nlen = snprintf(buf + len, sizeof(buf) - len, "%-3d - %-2d %-2d %-2d %-2d\r\n", j, thaco(CLASS_MAGIC_USER, j), thaco(CLASS_CLERIC, j), thaco(CLASS_THIEF, j), thaco(CLASS_WARRIOR, j));
                if(len + nlen >= sizeof(buf))
                {
                    break;
                }
...
static struct set_struct
{
    ...
    const char  type;
                  ...
                  {"age", LVL_GOD, BOTH, NUMBER},         {"align", LVL_BUILDER, BOTH, NUMBER},   {"bank", LVL_BUILDER, PC, NUMBER},
                  {"brief", LVL_GOD, PC, BINARY}, /* 5  */
                  {"cha", LVL_BUILDER, BOTH, NUMBER},     {"class", LVL_BUILDER, BOTH, MISC},     {"color", LVL_GOD, PC, BINARY},
                  {"con", LVL_BUILDER, BOTH, NUMBER},     {"damroll", LVL_BUILDER, BOTH, NUMBER}, /* 10 */
                  {"deleted", LVL_IMPL, PC, BINARY},      {"dex", LVL_BUILDER, BOTH, NUMBER},     {"drunk", LVL_BUILDER, BOTH, MISC},
                  {"exp", LVL_GOD, BOTH, NUMBER},         {"frozen", LVL_GRGOD, PC, BINARY}, /* 15 */
                  {"gold", LVL_BUILDER, BOTH, NUMBER},    {"height", LVL_BUILDER, BOTH, NUMBER},  {"hitpoints", LVL_BUILDER, BOTH, NUMBER},
                  {"hitroll", LVL_BUILDER, BOTH, NUMBER}, {"hunger", LVL_BUILDER, BOTH, MISC}, /* 20 */
                  {"int", LVL_BUILDER, BOTH, NUMBER},     {"invis", LVL_GOD, PC, NUMBER},         {"invstart", LVL_BUILDER, PC, BINARY},
                  {"killer", LVL_GOD, PC, BINARY},        {"level", LVL_GRGOD, BOTH, NUMBER}, /* 25 */
                  {"loadroom", LVL_BUILDER, PC, MISC},    {"mana", LVL_BUILDER, BOTH, NUMBER},    {"maxhit", LVL_BUILDER, BOTH, NUMBER},
                  {"maxmana", LVL_BUILDER, BOTH, NUMBER}, {"maxmove", LVL_BUILDER, BOTH, NUMBER}, /* 30 */
                  {"move", LVL_BUILDER, BOTH, NUMBER},    {"name", LVL_IMMORT, PC, MISC},         {"nodelete", LVL_GOD, PC, BINARY},
                  {"nohassle", LVL_GOD, PC, BINARY},      {"nosummon", LVL_BUILDER, PC, BINARY}, /* 35 */
                  {"nowizlist", LVL_GRGOD, PC, BINARY},   {"olc", LVL_GRGOD, PC, MISC},           {"password", LVL_GRGOD, PC, MISC},
                  {"poofin", LVL_IMMORT, PC, MISC},       {"poofout", LVL_IMMORT, PC, MISC}, /* 40 */
                  {"practices", LVL_GOD, PC, NUMBER},     {"quest", LVL_GOD, PC, BINARY},         {"room", LVL_BUILDER, BOTH, NUMBER},
                  {"screenwidth", LVL_GOD, PC, NUMBER},   {"sex", LVL_GOD, BOTH, MISC}, /* 45 */
                  {"showvnums", LVL_BUILDER, PC, BINARY}, {"siteok", LVL_GOD, PC, BINARY},        {"str", LVL_BUILDER, BOTH, NUMBER},
                  {"stradd", LVL_BUILDER, BOTH, NUMBER},  {"thief", LVL_GOD, PC, BINARY}, /* 50 */
                  {"thirst", LVL_BUILDER, BOTH, MISC},    {"title", LVL_GOD, PC, MISC},           {"variable", LVL_GRGOD, PC, MISC},
                  {"weight", LVL_BUILDER, BOTH, NUMBER},  {"wis", LVL_BUILDER, BOTH, NUMBER}, /* 55 */
                  {"questpoints", LVL_GOD, PC, NUMBER},   {"questhistory", LVL_GOD, PC, NUMBER},  {"\n", 0, BOTH, MISC}};
...
static void show_set_help(struct char_data *ch)
{
    ...
    for(i = 0; *(set_fields[i].cmd) != '\n'; i++)
    {
        ...
        {
            add_len = snprintf(
                buf + len, sizeof(buf) - len, "%-20s%-5s  %-4s  %-6s\r\n", set_fields[i].cmd, set_levels[((int)(set_fields[i].level) - LVL_IMMORT)], set_targets[(int)(set_fields[i].pcnpc) - 1], set_types[(int)(set_fields[i].type)]);
            len += add_len;
        }
    }
    page_string(ch->desc, buf, TRUE);
...
ACMD(do_zcheck)
{
    ...
    for(i = 0; i < top_of_mobt; i++)
    {
        ...
        { /*is mob in this zone?*/
            ...
            if(mob->mob_specials.damsizedice == 1 && mob->mob_specials.damnodice == 1 && GET_LEVEL(mob) == 0 && (found = 1))
            {
            ...
            if(found)
            {
                send_to_char(ch, "%s[%5d]%s %-30s: %s\r\n", CCCYN(ch, C_NRM), GET_MOB_VNUM(mob), CCYEL(ch, C_NRM), GET_NAME(mob), CCNRM(ch, C_NRM));
    ...
    for(i = 0; i < top_of_objt; i++)
    {
        ...
        { /*is object in this zone?*/
            ...
            {
                if((GET_OBJ_COST(obj) || (GET_OBJ_WEIGHT(obj) && GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN) || GET_OBJ_RENT(obj)) && (found = 1))
                {
                    len += snprintf(buf + len, sizeof(buf) - len, "- is NO_TAKE, but has cost (%d) weight (%d) or rent (%d) set.\r\n", GET_OBJ_COST(obj), GET_OBJ_WEIGHT(obj), GET_OBJ_RENT(obj));
                }
            }
            else
            {
            ...
            if(obj->action_description && *obj->action_description && GET_OBJ_TYPE(obj) != ITEM_STAFF && GET_OBJ_TYPE(obj) != ITEM_WAND && GET_OBJ_TYPE(obj) != ITEM_SCROLL && GET_OBJ_TYPE(obj) != ITEM_NOTE && (found = 1))
            {
                len += snprintf(buf + len, sizeof(buf) - len, "- has action_description set, but is inappropriate type.\r\n");
            }
            ...
            for(j = 0; j < MAX_OBJ_AFFECT; j++)
            {
                if(zaffs[(int)obj->affected[j].location].max_aff != -99 && /* only care if a range is set */
                   (obj->affected[j].modifier > zaffs[(int)obj->affected[j].location].max_aff || obj->affected[j].modifier < zaffs[(int)obj->affected[j].location].min_aff ||
        ...
        if(world[i].zone == zrnum)
        {
            for(j = 0; j < DIR_COUNT; j++)
            {
                ...
                for(k = 0; offlimit_zones[k] != -1; k++)
                {
                    ...
                    {
                        len += snprintf(buf + len, sizeof(buf) - len, "- Exit %s cannot connect to %d (zone off limits).\r\n", dirs[j], world[exroom].number);
                    }
                } /* for (k.. */
            } /* cycle directions */
            ...
            {
                len += snprintf(buf + len, sizeof(buf) - len, "- Room description is too short. (%4.4d of min. %d characters).\r\n", (int)strlen(world[i].description), MIN_ROOM_DESC_LENGTH);
            }

            if(strncmp(world[i].description, "   ", 3) && (found = 1))
            {
                len += snprintf(buf + len, sizeof(buf) - len, "- Room description not formatted with indent (/fi in the editor).\r\n");
            }
            ...
            {
                len += snprintf(buf + len, sizeof(buf) - len, "- Room description not wrapped at %d chars (/fi in the editor).\r\n", MAX_COLOUMN_WIDTH);
            }
        ...
        for(cmd_no = 0; ZCMD2.command != 'S'; cmd_no++)
        {
            switch(ZCMD2.command)
            {
                ...
                case 'G': /* obj_to_char */
                    if(ZCMD2.arg1 == ornum)
                    {
                        send_to_char(ch, "  [%5d] %s (Given to %s [%d][%d Max])\r\n", lastroom_v, world[lastroom_r].name, mob_proto[lastmob_r].player.short_descr, mob_index[lastmob_r].vnum, ZCMD2.arg2);
                    }
                    break;
...
static void trg_checkload(struct char_data *ch, trig_vnum tvnum)
{
    ...
    send_to_char(
        ch, "Checking load info for the %s trigger '%s':\r\n", trig_index[trnum]->proto->attach_type == MOB_TRIGGER ? "mobile" : (trig_index[trnum]->proto->attach_type == OBJ_TRIGGER ? "object" : "room"), trig_index[trnum]->proto->name);
        ...
        for(cmd_no = 0; ZCMD2.command != 'S'; cmd_no++)
        {
            switch(ZCMD2.command)
            {
                ...
                case 'T': /* trigger to something */
                    ...
                    {
                        send_to_char(ch, "mob [%5d] %-60s (zedit room %5d)\r\n", mob_index[lastmob_r].vnum, mob_proto[lastmob_r].player.short_descr, lastroom_v);
                        found = 1;
                    }
...
ACMD(do_plist)
{
    ...
    for(i = 0; i <= top_of_p_table; i++)
    {
        ...
        len += snprintf(buf + len, sizeof(buf) - len, "[%3ld] (%2d) %c%-15s %s\r\n", player_table[i].id, player_table[i].level, UPPER(*player_table[i].name), player_table[i].name + 1, timestr);
        count++;
    }
    snprintf(buf + len,
             sizeof(buf) - len,
...
bool change_player_name(struct char_data *ch, struct char_data *vict, char *new_name)
{
    ...
    if(!new_name || !(*new_name) || strlen(new_name) < 2 || strlen(new_name) > MAX_NAME_LENGTH || !valid_name(new_name) || fill_word(new_name) || reserved_word(new_name))
    {
    ...
    {
        send_to_char(vict, "Your login name has changed from %s%s%s to %s%s%s.\r\n", CCYEL(vict, C_NRM), old_name, CCNRM(vict, C_NRM), CCYEL(vict, C_NRM), new_name, CCNRM(vict, C_NRM));
    }
    ...
    if(is_abbrev(arg, "list"))
    {
        ...
        for(zn = 0; zn <= top_of_zone_table; zn++)
        {
            if(ZONE_FLAGGED(zn, ZONE_NOBUILD))
            {
                ...
                send_to_char(
                    ch, "[%s%3d%s] %s%-*s %s%-1s%s\r\n", QGRN, zone_table[zn].number, QNRM, QCYN, count_color_chars(zone_table[zn].name) + 30, zone_table[zn].name, QYEL, zone_table[zn].builders ? zone_table[zn].builders : "None.", QNRM);
                counter++;
            }
        }
        if(counter == 0)
...
ACMD(do_oset)
{
    ...
    else if(!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying)) && !(obj = get_obj_in_list_vis(ch, arg, NULL, world[IN_ROOM(ch)].contents)))
    {
