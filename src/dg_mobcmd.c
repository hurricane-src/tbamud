...
#define MOB_OR_IMPL(ch) ((IS_NPC(ch) && (!(ch)->desc || GET_LEVEL((ch)->desc->original) >= LVL_IMPL)) || (SCRIPT(ch) && TRIGGERS(SCRIPT(ch))))
...
ACMD(do_masound)
{
    ...
    for(door = 0; door < DIR_COUNT; door++)
    {
        ...
        if(((newexit = world[was_in_room].dir_option[door]) != NULL) && newexit->to_room != NOWHERE && newexit->to_room != was_in_room)
        {
