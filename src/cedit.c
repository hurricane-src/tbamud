...
ACMD(do_oasis_cedit)
{
    ...
    if(!*buf1)
    {
        ...
        mudlog(BRF, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "OLC: %s starts editing the game configuration.", GET_NAME(ch));
        return;
    }
        ...
        case CEDIT_CONFIRM_SAVESTRING:
            switch(*arg)
            {
                ...
                case 'Y':
                    ...
                    mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s modifies the game configuration.", GET_NAME(d->character));
