        ...
        if(IS_SET_AR((mob_proto[num].char_specials.saved.act), mob_flag))
        {
            ...
            if((mob = read_mobile(num, REAL)) != NULL)
            {
                ...
                len += snprintf(buf + len,
                                sizeof(buf) - len,
                                "%s%3d. %s[%s%5d%s]%s Level %s%-3d%s %s%s\r\n",
                                CCNRM(ch, C_NRM),
                                ++found,
                                CCCYN(ch, C_NRM),
                                CCYEL(ch, C_NRM),
    ...
    for(num = 0; num <= top_of_mobt; num++)
    {
        if((mob_proto[num].player.level) == mob_level)
        {
            if((mob = read_mobile(num, REAL)) != NULL)
            {
                char_to_room(mob, 0);
                len += snprintf(
                    buf + len, sizeof(buf) - len, "%s%3d. %s[%s%5d%s]%s %s%s\r\n", CCNRM(ch, C_NRM), ++found, CCCYN(ch, C_NRM), CCYEL(ch, C_NRM), GET_MOB_VNUM(mob), CCCYN(ch, C_NRM), CCNRM(ch, C_NRM), GET_NAME(mob), CCNRM(ch, C_NRM));
                extract_char(mob); /* Finished with the mob - remove it from the MUD */
                if(len > sizeof(buf))
                {
                    break;
...
static void perform_obj_aff_list(struct char_data *ch, char *arg)
{
    ...
    { /* olist affect 8 is AC-Apply for Armor */
        for(num = 0; num <= top_of_objt; num++)
        {
            if((apply == APPLY_CLASS && obj_proto[num].obj_flags.type_flag == ITEM_WEAPON) || (apply == APPLY_LEVEL && obj_proto[num].obj_flags.type_flag == ITEM_ARMOR))
            {
                ov = obj_index[num].vnum;
    ...
    for(i = 0; i < MAX_OBJ_LIST; i++)
    {
        if((r_num = real_object(lst[i].vobj)) != NOTHING)
        {
            tmp_len = snprintf(buf + len,
                               sizeof(buf) - len,
                               "%s%3d%s) %s[%s%8d%s] %s%3d %s%-*s %s[%s]%s%s\r\n",
                               QGRN,
                               ++found,
                               QNRM,
                               QCYN,
...
static void perform_obj_name_list(struct char_data *ch, char *arg)
{
    ...
    for(num = 0; num <= top_of_objt; num++)
    {
        if(is_name(arg, obj_proto[num].name))
        {
            ...
            tmp_len = snprintf(buf + len,
                               sizeof(buf) - len,
                               "%s%4d%s) %s[%s%5d%s] %s(%s%3d%s)%s %-*s%s [%s]%s%s\r\n",
                               QGRN,
                               ++found,
                               QNRM,
                               QCYN,
        ...
        if(GET_ROOM_VNUM(nr) >= first)
        {
            for(j = 0; j < DIR_COUNT; j++)
            {
                if(world[nr].dir_option[j])
                {
                    ...
                    {
                        send_to_char(ch, "%3d %-30s%s at %5d (%-5s) ---> %5d\r\n", zone_table[world[to_room].zone].number, zone_table[world[to_room].zone].name, QNRM, GET_ROOM_VNUM(nr), dirs[j], world[to_room].number);
                    }
                }
            }
        }
...
static void list_rooms(struct char_data *ch, zone_rnum rnum, room_vnum vmin, room_vnum vmax)
{
    ...
    for(i = 0; i <= top_of_world; i++)
    {
        ...
        if((world[i].number >= bottom) && (world[i].number <= top))
        {
            ...
            len += snprintf(buf + len, sizeof(buf) - len, "%4d) [%s%-5d%s] %s%-*s%s %s", counter, QGRN, world[i].number, QNRM, QCYN, count_color_chars(world[i].name) + 44, world[i].name, QNRM, world[i].proto_script ? "[TRIG] " : "");
    ...
    for(i = 0; i <= top_of_objt; i++)
    {
        if(obj_index[i].vnum >= bottom && obj_index[i].vnum <= top)
        {
            ...
            len += snprintf(buf + len,
                            ...
                            QNRM,
                            QCYN,
                            count_color_chars(obj_proto[i].short_description) + 44,
...
static void list_shops(struct char_data *ch, zone_rnum rnum, shop_vnum vmin, shop_vnum vmax)
{
    ...
    for(i = 0; i <= top_shop; i++)
    {
        if(SHOP_NUM(i) >= bottom && SHOP_NUM(i) <= top)
        {
            ...
            send_to_char(ch, "%s%4d%s) [%s%-5d%s] [%s%-5d%s]", QGRN, counter, QNRM, QGRN, SHOP_NUM(i), QNRM, QGRN, i + 1, QNRM);
        ...
        if(zone_table[i].number >= bottom && zone_table[i].number <= top)
        {
            if((!use_name) || (is_name(name, zone_table[i].builders)))
            {
                ...
                tmp_len = snprintf(buf + len,
                                   ...
                                   QNRM,
                                   QCYN,
                                   count_color_chars(zone_table[i].name) + 30,
                                   zone_table[i].name,
                                   QYEL,
...
void print_zone(struct char_data *ch, zone_vnum vnum)
{
    ...
    send_to_char(ch,
                 ...
                 QGRN,
                 QCYN,
                 zone_table[rnum].reset_mode ? ((zone_table[rnum].reset_mode == 1) ? "Reset when no players are in zone." : "Normal reset.") : "Never reset",
                 QGRN,
                 QCYN,
                 buf,
                 QGRN,
...
static void list_triggers(struct char_data *ch, zone_rnum rnum, trig_vnum vmin, trig_vnum vmax)
{
    ...
    for(i = 0; i < top_of_trigt; i++)
    {
        ...
        if((trig_index[i]->vnum >= bottom) && (trig_index[i]->vnum <= top))
        {
            ...
            send_to_char(ch, "%4d) [%s%5d%s] %s%-45.45s%s ", counter, QGRN, trig_index[i]->vnum, QNRM, QCYN, trig_index[i]->proto->name, QNRM);
