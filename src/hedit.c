...
ACMD(do_oasis_hedit)
{
    ...
    mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s starts editing help files.", GET_NAME(d->character));
}
        ...
        case HEDIT_CONFIRM_EDIT:
            switch(*arg)
            {
                ...
                case 'N':
                    ...
                    {
                        write_to_output(d, "Do you wish to edit the '%s' help file? ", help_table[OLC_ZNUM(d)].keywords);
                        OLC_MODE(d) = HEDIT_CONFIRM_EDIT;
                    }
                    break;
                default:
...
ACMD(do_helpcheck)
{
    ...
    for(i = 1; *(complete_cmd_info[i].command) != '\n'; i++)
    {
        if(complete_cmd_info[i].command_pointer != do_action && complete_cmd_info[i].minimum_level >= 0)
        {
            ...
            {
                nlen = snprintf(buf + len, sizeof(buf) - len, "%-20.20s%s", complete_cmd_info[i].command, (++count % 3 ? "" : "\r\n"));
...
ACMD(do_hindex)
{
    ...
    for(i = 0; i < top_of_helpt; i++)
    {
        ...
        {
            len += snprintf(buf + len, sizeof(buf) - len, "%-20.20s%s", help_table[i].keywords, (++count % 3 ? "" : "\r\n"));
        }
        else if(strstr(help_table[i].keywords, argument) && (GET_LEVEL(ch) >= help_table[i].min_level))
        {
            len2 += snprintf(buf2 + len2, sizeof(buf2) - len2, "%-20.20s%s", help_table[i].keywords, (++count2 % 3 ? "" : "\r\n"));
        }
    }
    if(count % 3)
    {
