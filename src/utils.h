...
#define STRENGTH_APPLY_INDEX(ch) (((GET_ADD(ch) == 0) || (GET_STR(ch) != 18)) ? GET_STR(ch) : (GET_ADD(ch) <= 50) ? 26 : ((GET_ADD(ch) <= 75) ? 27 : ((GET_ADD(ch) <= 90) ? 28 : ((GET_ADD(ch) <= 99) ? 29 : 30))))
...
#define SENDOK(ch)                                  (((ch)->desc || SCRIPT_CHECK((ch), MTRIG_ACT)) && (to_sleeping || AWAKE(ch)) && !PLR_FLAGGED((ch), PLR_WRITING))
...
#define LIGHT_OK(sub)                               (!AFF_FLAGGED(sub, AFF_BLIND) && (IS_LIGHT(IN_ROOM(sub)) || AFF_FLAGGED((sub), AFF_INFRAVISION) || GET_LEVEL(sub) >= LVL_IMMORT))

/** Defines if sub character can see the invisible obj character. */
#define INVIS_OK(sub, obj)                          ((!AFF_FLAGGED((obj), AFF_INVISIBLE) || AFF_FLAGGED(sub, AFF_DETECT_INVIS)) && (!AFF_FLAGGED((obj), AFF_HIDE) || AFF_FLAGGED(sub, AFF_SENSE_LIFE)))
...
#define CAN_SEE(sub, obj)                           (SELF(sub, obj) || ((GET_REAL_LEVEL(sub) >= (IS_NPC(obj) ? 0 : GET_INVIS_LEV(obj))) && IMM_CAN_SEE(sub, obj)))
/* End of CAN_SEE */
...
#define CAN_GO(ch, door)                            (EXIT(ch, door) && (EXIT(ch, door)->to_room != NOWHERE) && !IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED))
