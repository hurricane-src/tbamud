...
static void aedit_save_to_disk(struct descriptor_data *d)
{
    ...
    for(i = 0; i <= top_of_socialt; i++)
    {
        ...
        sprintf(buf, "%s\n%s\n\n", ((soc_mess_list[i].char_obj_found) ? soc_mess_list[i].char_obj_found : "#"), ((soc_mess_list[i].others_obj_found) ? soc_mess_list[i].others_obj_found : "#"));
        fprintf(fp, "%s", convert_from_tabs(buf));
    }
        ...
        case AEDIT_CONFIRM_SAVESTRING:
            switch(*arg)
            {
                ...
                case 'Y':
                    ...
                    mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s edits action %s", GET_NAME(d->character), OLC_ACTION(d)->command);
