...
void save_ibt_file(int mode)
{
    ...
    else
    {
        for(ibtData = first_ibt; ibtData; ibtData = ibtData->next)
        {
            ...
            fprintf(fp, "Level     %d\n", ibtData->level);
            fprintf(fp, "Room      %d\n", ibtData->room);
            fprintf(fp, "Flags     %d %d %d %d\n", ibtData->flags[0], ibtData->flags[1], ibtData->flags[2], ibtData->flags[3]);
            fprintf(fp, "End\n");
        }
        fclose(fp);
        return;
...
ACMD(do_ibt)
{
        ...
        if(first_ibt)
        {
            ...
            for(ibtData = first_ibt; ibtData; ibtData = ibtData->next)
            {
                ...
                if(IBT_FLAGGED(ibtData, IBT_RESOLVED))
                {
                    ...
                    {
                        len += snprintf(
                            buf + len, sizeof(buf) - len, "%s%s%3d%s|%s%-12s%s|%s%6d%s|%s%5d%s|%s%s%s\r\n", imp, QGRN, i, QGRN, QGRN, ibtData->name, QGRN, QGRN, ibtData->room, QGRN, QGRN, ibtData->level, QGRN, QGRN, ibtData->text, QNRM);
                    }
                    num_res++;
                }
                else if(IBT_FLAGGED(ibtData, IBT_INPROGRESS))
                {
                    if(GET_LEVEL(ch) < LVL_IMMORT)
                    {
                        len += snprintf(buf + len, sizeof(buf) - len, "%s%s%3d%s|%s%s%s\r\n", imp, QYEL, i, QGRN, QYEL, ibtData->text, QNRM);
                    }
                    else
                    {
                        len += snprintf(
                            buf + len, sizeof(buf) - len, "%s%s%3d%s|%s%-12s%s|%s%6d%s|%s%5d%s|%s%s%s\r\n", imp, QYEL, i, QGRN, QYEL, ibtData->name, QGRN, QYEL, ibtData->room, QGRN, QYEL, ibtData->level, QGRN, QYEL, ibtData->text, QNRM);
                    }
                    num_unres++;
                }
                else
                {
                    ...
                    {
                        len += snprintf(
                            buf + len, sizeof(buf) - len, "%s%s%3d%s|%s%-12s%s|%s%6d%s|%s%5d%s|%s%s%s\r\n", imp, QRED, i, QGRN, QRED, ibtData->name, QGRN, QRED, ibtData->room, QGRN, QRED, ibtData->level, QGRN, QRED, ibtData->text, QNRM);
                    }
                    num_unres++;
                }
                if(len > sizeof(buf))
    ...
    else if(is_abbrev(arg, "submit"))
    {
        ...
        mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), FALSE, "%s has posted %s %s!", GET_NAME(ch), TANA(CMD_NAME), CMD_NAME);
        return;
    }
...
ACMD(do_oasis_ibtedit)
{
    ...
    mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "OLC: %s starts editing %s %d", GET_NAME(ch), IBT_TYPE, OLC_NUM(d));
}
...
static void ibtedit_disp_main_menu(struct descriptor_data *d)
{
    ...
    send_to_char(ch,
                 ...
                 nrm,
                 yel,
                 OLC_IBT(d)->body ? OLC_IBT(d)->body : "<Not Set!>",
                 yel,
                 nrm,
                 yel,
                 OLC_IBT(d)->notes ? OLC_IBT(d)->notes : "<Not Set!>",
                 yel,
                 nrm,
                 IBT_TYPE);
