...
static int handle_oset(struct obj_data *obj, char *argument)
{
    ...
    } handler[] = {{"alias", oset_alias}, {"apply", oset_apply}, {"longdesc", oset_long_description}, {"shortdesc", oset_short_description}, {"\n", NULL}};
            ...
            *str = '\x1';
            ...
            switch(LOWER(*field))
            {
                ...
                case 'p':
                    ...
                    break;
                case 'q':
                    if(!IS_NPC(c) && (!str_cmp(field, "questpoints") || !str_cmp(field, "qp") || !str_cmp(field, "qpnts")))
                    {
                        if(subfield && *subfield)
                        {
                            int addition = atoi(subfield);
...
/* see note in dg_scripts.h */
                        ...
                        snprintf(str, slen, "%c%ld", UID_CHAR, (IN_ROOM(c) != NOWHERE) ? room_script_id(world + IN_ROOM(c)) : ROOM_ID_BASE);
#else
        ...
        else if(r)
        {
            ...
            if(r->number == 0)
            {
                ...
                {
                    *str = '\0';
                    script_log("Trigger: %s, Vnum %d, type %d. Trying to access Global var list of void. Apparently this has not been set up!", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type);
                }
                else
                {
