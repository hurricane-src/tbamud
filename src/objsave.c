...
int objsave_save_obj_record(struct obj_data *obj, FILE *fp, int locate)
{
    ...
    if(GET_OBJ_VAL(obj, 0) != GET_OBJ_VAL(temp, 0) || GET_OBJ_VAL(obj, 1) != GET_OBJ_VAL(temp, 1) || GET_OBJ_VAL(obj, 2) != GET_OBJ_VAL(temp, 2) || GET_OBJ_VAL(obj, 3) != GET_OBJ_VAL(temp, 3))
    {
...
#define TEST_OBJN(field)             (obj->obj_flags.field != temp->obj_flags.field)
    ...
    for(counter2 = 0; counter2 < MAX_OBJ_AFFECT; counter2++)
    {
        ...
        {
            fprintf(fp, "Aff : %d %d %d\n", counter2, obj->affected[counter2].location, obj->affected[counter2].modifier);
        }
    }
    ...
    if(obj->ex_description || temp->ex_description)
    {
        /* To be reimplemented.  Need to handle this case in loading as
           well */
        if((obj->ex_description && temp->ex_description && obj->ex_description != temp->ex_description) || !obj->ex_description || !temp->ex_description)
        {
    ...
    { /* Was wearing it. */
        ...
        { /* Wearable. */
            ...
            { /* Oops, saved a player with double equipment? */
                mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "SYSERR: autoeq: '%s' already equipped in position %d.", GET_NAME(ch), location);
                location = LOC_INVENTORY;
            }
        }
    }
...
static int objsave_write_rentcode(FILE *fl, int rentcode, int cost_per_day, struct char_data *ch)
{
    if(fprintf(fl, "%d %ld %d %d %d %d\r\n", rentcode, (long)time(0), cost_per_day, GET_GOLD(ch), GET_BANK_GOLD(ch), 0) < 1)
    {
...
static int Crash_offer_rent(struct char_data *ch, struct char_data *recep, int display, int factor)
{
    ...
    if(display)
    {
        ...
        act(buf, FALSE, recep, 0, ch, TO_VICT);
        sprintf(buf, "$n tells you, 'For a total of %ld coins%s.'", totalcost, (factor == RENT_FACTOR ? " per day" : ""));
        act(buf, FALSE, recep, 0, ch, TO_VICT);
...
static int Crash_load_objs(struct char_data *ch)
{
    ...
    switch(orig_rent_code = rentcode)
    {
        ...
        case RENT_TIMEDOUT:
            ...
            break;
        default:
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "WARNING: %s entering game with undefined rent code.", GET_NAME(ch));
            break;
    }
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "%s (level %d) has %d object%s (max %d).", GET_NAME(ch), GET_LEVEL(ch), num_objs, num_objs != 1 ? "s" : "", CONFIG_MAX_OBJ_SAVE);
