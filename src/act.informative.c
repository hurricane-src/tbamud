...
static void list_obj_to_char(struct obj_data *list, struct char_data *ch, int mode, int show)
{
    ...
    for(i = list; i; i = i->next_content)
    {
        ...
        {
            if((j->short_description == i->short_description && j->name == i->name) || (!strcmp(j->short_description, i->short_description) && !strcmp(j->name, i->name)))
            {
        ...
        if(num > 0 && (mode != SHOW_OBJ_LONG || *display->description != '.' || (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_HOLYLIGHT))))
        {
...
static void list_one_char(struct char_data *i, struct char_data *ch)
{
    ...
    const char      *positions[] = {
        " is lying here, dead.", " is lying here, mortally wounded.", " is lying here, incapacitated.", " is lying here, stunned.", " is sleeping here.", " is resting here.", " is sitting here.", "!FIGHTING!", " is standing here."};
    ...
    if(GROUP(i))
    {
        if(GROUP(i) == GROUP(ch))
        {
            send_to_char(ch, "(%s%s%s) ", CBGRN(ch, C_NRM), GROUP_LEADER(GROUP(i)) == i ? "leader" : "group", CCNRM(ch, C_NRM));
        }
        else
        {
            send_to_char(ch, "(%s%s%s) ", CBRED(ch, C_NRM), GROUP_LEADER(GROUP(i)) == i ? "leader" : "group", CCNRM(ch, C_NRM));
        }
    }
...
ACMD(do_exits)
{
    ...
    for(door = 0; door < DIR_COUNT; door++)
    {
        ...
        {
            send_to_char(ch, "%-5s -[%5d]%s %s\r\n", dirs[door], GET_ROOM_VNUM(EXIT(ch, door)->to_room), EXIT_FLAGGED(EXIT(ch, door), EX_HIDDEN) ? "[HIDDEN]" : "", world[EXIT(ch, door)->to_room].name);
        }
        else if(CONFIG_DISP_CLOSED_DOORS && EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED))
        {
            /*But we tell them the door is closed */
            send_to_char(ch, "%-5s - The %s is closed%s\r\n", dirs[door], (EXIT(ch, door)->keyword) ? fname(EXIT(ch, door)->keyword) : "opening", EXIT_FLAGGED(EXIT(ch, door), EX_HIDDEN) ? " and hidden." : ".");
        }
        else
        {
...
#define WHO_FORMAT "Usage: who [minlev[-maxlev]] [-n name] [-c classlist] [-k] [-l] [-n] [-q] [-r] [-s] [-z]\r\n"
...
ACMD(do_who)
{
    ...
    for(i = 0; *rank[i].disp != '\n'; i++)
    {
        ...
        for(d = descriptor_list; d; d = d->next)
        {
            ...
            if(short_list)
            {
                send_to_char(ch, "%s[%2d %s] %-12.12s%s%s", (GET_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""), GET_LEVEL(tch), CLASS_ABBR(tch), GET_NAME(tch), CCNRM(ch, C_SPR), ((!(++num_can_see % 4)) ? "\r\n" : ""));
            }
            else
            {
                num_can_see++;
                send_to_char(ch, "%s[%2d %s] %s%s%s%s", (GET_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""), GET_LEVEL(tch), CLASS_ABBR(tch), GET_NAME(tch), (*GET_TITLE(tch) ? " " : ""), GET_TITLE(tch), CCNRM(ch, C_SPR));
...
static void perform_immort_where(struct char_data *ch, char *arg)
{
        ...
        for(d = descriptor_list; d; d = d->next)
        {
            if(IS_PLAYING(d))
            {
                ...
                if(i && CAN_SEE(ch, i) && (IN_ROOM(i) != NOWHERE))
                {
                    if(d->original)
                    {
                        send_to_char(ch, "%-8s%s - [%5d] %s%s (in %s%s)\r\n", GET_NAME(i), QNRM, GET_ROOM_VNUM(IN_ROOM(d->character)), world[IN_ROOM(d->character)].name, QNRM, GET_NAME(d->character), QNRM);
                    }
                    else
                    {
                        send_to_char(ch,
    ...
    else
    {
        for(i = character_list; i; i = i->next)
        {
            ...
            {
                found = 1;
                send_to_char(ch, "M%3d. %-25s%s - [%5d] %-25s%s", ++num, GET_NAME(i), QNRM, GET_ROOM_VNUM(IN_ROOM(i)), world[IN_ROOM(i)].name, QNRM);
                if(SCRIPT(i) && TRIGGERS(SCRIPT(i)))
                {
                    if(!TRIGGERS(SCRIPT(i))->next)
                    {
...
ACMD(do_levels)
{
    ...
    {
        nlen = snprintf(buf + len, sizeof(buf) - len, "[%2d] %8d-%-8d : ", (int)i, level_exp(GET_CLASS(ch), i), level_exp(GET_CLASS(ch), i + 1) - 1);
...
ACMD(do_whois)
{
    ...
    send_to_char(ch, "Name: %s %s\r\nSex: %s\r\n", GET_NAME(victim), (victim->player.title ? victim->player.title : ""), buf);
    ...
    if(!(GET_LEVEL(victim) < LVL_IMMORT) || (GET_LEVEL(ch) >= GET_LEVEL(victim)))
    {
        ...
        if(!got_from_file)
        {
            send_to_char(ch, "Last Logon: Playing now!  (Idle %d Minutes)", victim->char_specials.timer * SECS_PER_MUD_HOUR / SECS_PER_REAL_MIN);
...
ACMD(do_areas)
{
    ...
    { /* Go through the whole zone table */
        ...
        if(show_zone)
        {
            ...
            tmp_len = snprintf(
                buf + len, sizeof(buf) - len, "\tn(%3d) %s%-*s\tn %s%s\tn\r\n", ++zcount, overlap ? QRED : QCYN, count_color_chars(zone_table[i].name) + 30, zone_table[i].name, lev_set ? "\tc" : "\tn", lev_set ? lev_str : "All Levels");
            len += tmp_len;
        }
    }
...
ACMD(do_scan)
{
    ...
    for(door = 0; door < DIR_COUNT; door++)
    {
        ...
        {
            if(world[scanned_room].dir_option[door] && world[scanned_room].dir_option[door]->to_room != NOWHERE && !IS_SET(world[scanned_room].dir_option[door]->exit_info, EX_CLOSED) &&
