...
int delete_room(room_rnum rnum)
{
    ...
    do
    {
        ...
        for(j = 0; j < DIR_COUNT; j++)
        {
            ...
            {
                if((!W_EXIT(i, j)->keyword || !*W_EXIT(i, j)->keyword) && (!W_EXIT(i, j)->general_description || !*W_EXIT(i, j)->general_description))
                {
...
int save_rooms(zone_rnum rzone)
{
    ...
    log("GenOLC: save_rooms: Saving rooms in zone #%d (%d-%d).", zone_table[rzone].number, genolc_zone_bottom(rzone), zone_table[rzone].top);
    ...
    for(i = genolc_zone_bottom(rzone); i <= zone_table[rzone].top; i++)
    {
        ...
        if((rnum = real_room(i)) != NOWHERE)
        {
            ...
            {
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Could not save room #%d due to size (%d > maximum of %d).", room->number, n, MAX_STRING_LENGTH);
                continue;
            }
