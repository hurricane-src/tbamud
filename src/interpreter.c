...
static int         sort_commands_helper(const void *a, const void *b) { return strcmp(complete_cmd_info[*(const int *)a].sort_as, complete_cmd_info[*(const int *)b].sort_as); }
 ...
 * ignores fill words */
char *two_arguments(char *argument, char *first_arg, char *second_arg) { return (one_argument(one_argument(argument, first_arg), second_arg)); /* :-) */ }
...
void nanny(struct descriptor_data *d, char *arg)
{
    ...
    switch(STATE(d))
    {
        ...
        case CON_QCLASS:
            ...
            if(d->olc)
            { /// @note 20250321 -- free(NULL) is a NOP, this if is useless
                free(d->olc);
                d->olc = NULL;
            }
        ...
        case CON_DELCNF2:
            if(!strcmp(arg, "yes") || !strcmp(arg, "YES"))
            {
                ...
                mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(d->character)), TRUE, "%s (lev %d) has self-deleted.", GET_NAME(d->character), GET_LEVEL(d->character));
                STATE(d) = CON_CLOSE;
                return;
            }
            else
