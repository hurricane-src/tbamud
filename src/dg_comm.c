        ...
        switch(*p)
        {
            ...
            case '*':
                ...
                *s = '\0';
                ...
                otokens[i] = find_invis ? (void *)get_char_in_room(&world[IN_ROOM(ch)], name) : (void *)get_char_room_vis(ch, name, NULL);
                tokens[++i] = ++s;
                break;
...
void send_to_zone(char *messg, zone_rnum zone)
{
    ...
    {
        if(!i->connected && i->character && AWAKE(i->character) && (IN_ROOM(i->character) != NOWHERE) && (world[IN_ROOM(i->character)].zone == zone))
        {
