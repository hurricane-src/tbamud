...
{
    char **textfiles[] = {&wizlist, &immlist, &news, &credits, &motd, &imotd, &help, &ihelp, &info, &policies, &handbook, &background, &GREETINGS, &bugs, &typos, &ideas, NULL};
    int    rf;
...
void boot_db(void)
{
    ...
    {
        log("Resetting #%d: %s (rooms %d-%d).", zone_table[i].number, zone_table[i].name, zone_table[i].bot, zone_table[i].top);
        reset_zone(i);
    }
...
void index_boot(int mode)
{
    ...
    for(line_number = 0;; ++line_number)
    {
        ...
        if(fscanf(db_index, "%s\n", buf1) != 1)
        {
            ...
            {
                log("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix, index_filename, strerror(errno));
            }
            else
            {
                log("SYSERR: boot error -- error parsing index file %s%s on line %d", prefix, index_filename, line_number);
            }
            exit(1);
        }
    ...
    for(line_number = 1;; ++line_number)
    {
        if(fscanf(db_index, "%s\n", buf1) != 1)
        {
            if(feof(db_index))
            {
                log("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s", prefix, index_filename);
            }
            else if(ferror(db_index))
            {
                log("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix, index_filename, strerror(errno));
    ...
    for(zone = 0; zone <= top_of_zone_table; zone++)
    {
        for(cmd_no = 0; ZCMD.command != 'S'; cmd_no++)
        {
            ...
            if(a == NOWHERE || b == NOWHERE || c == NOWHERE)
            {
                if(!mini_mud)
                {
                    snprintf(buf, sizeof(buf), "Invalid vnum %d, cmd disabled", a == NOWHERE ? olda : b == NOWHERE ? oldb : oldc);
...
static void load_zones(FILE *fl, char *zonename)
{
    ...
    if(sscanf(buf, " %hd %hd %d %d %s %s %s %s %d %d", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode, zbuf1, zbuf2, zbuf3, zbuf4, &Z.min_level, &Z.max_level) != 10)
    {
...
int vnum_mobile(char *searchname, struct char_data *ch)
{
    ...
    for(nr = 0; nr <= top_of_mobt; nr++)
    {
        ...
        {
            send_to_char(ch, "%3d. [%5d] %-40s %s\r\n", ++found, mob_index[nr].vnum, mob_proto[nr].player.short_descr, mob_proto[nr].proto_script ? "[TRIG]" : "");
        }
    }
...
int vnum_object(char *searchname, struct char_data *ch)
{
    ...
    for(nr = 0; nr <= top_of_objt; nr++)
    {
        ...
        {
            send_to_char(ch, "%3d. [%5d] %-40s %s\r\n", ++found, obj_index[nr].vnum, obj_proto[nr].short_description, obj_proto[nr].proto_script ? "[TRIG]" : "");
        }
    }
...
int vnum_room(char *searchname, struct char_data *ch)
{
    ...
    for(nr = 0; nr <= top_of_world; nr++)
    {
        ...
        {
            send_to_char(ch, "%3d. [%5d] %-40s %s\r\n", ++found, world[nr].number, world[nr].name, world[nr].proto_script ? "[TRIG]" : "");
        }
    }
    return (found);
}
...
void zone_update(void)
{
    ...
    for(update_u = reset_q.head; update_u; update_u = update_u->next)
    {
        if(zone_table[update_u->zone_to_reset].reset_mode == 2 || is_empty(update_u->zone_to_reset))
        {
            ...
            mudlog(CMP, LVL_IMPL + 1, FALSE, "Auto zone reset: %s (Zone %d)", zone_table[update_u->zone_to_reset].name, zone_table[update_u->zone_to_reset].number);
            struct descriptor_data *pt;
            for(pt = descriptor_list; pt; pt = pt->next)
            {
                if(IS_PLAYING(pt) && pt->character && PRF_FLAGGED(pt->character, PRF_ZONERESETS))
                {
                    send_to_char(pt->character, "%s[Auto zone reset: %s (Zone %d)]%s", CCGRN(pt->character, C_NRM), zone_table[update_u->zone_to_reset].name, zone_table[update_u->zone_to_reset].number, CCNRM(pt->character, C_NRM));
                }
            }
            /* dequeue */
...
static void log_zone_error(zone_rnum zone, int cmd_no, const char *message)
{
    ...
    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: ...offending cmd: '%c' cmd in zone #%d, line %d", ZCMD.command, zone_table[zone].number, ZCMD.line);
}
...
void init_char(struct char_data *ch)
{
    ...
    if(ch->desc)
    {
        if(ch->desc->pProtocol->pVariables[eMSDP_ANSI_COLORS] || ch->desc->pProtocol->pVariables[eMSDP_XTERM_256_COLORS])
        {
...
static int check_object(struct obj_data *obj)
{
    ...
    {
        log("SYSERR: Object #%d (%s) has negative cost/day (%d).", GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_RENT(obj));
    }
    ...
    switch(GET_OBJ_TYPE(obj))
    {
        case ITEM_DRINKCON:
        {
            ...
            {
                log("SYSERR: Object #%d (%s) doesn't have drink type as last keyword. (%s)", GET_OBJ_VNUM(obj), obj->short_description, obj->name);
            }
        }
        /* Fall through. */
        case ITEM_FOUNTAIN:
            ...
            {
                log("SYSERR: Object #%d (%s) contains (%d) more than maximum (%d).", GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 0));
            }
            break;
        case ITEM_SCROLL:
        case ITEM_POTION:
        ...
        case ITEM_STAFF:
            ...
            {
                log("SYSERR: Object #%d (%s) has more charges (%d) than maximum (%d).", GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 1));
            }
            break;
        case ITEM_NOTE:
            if(obj->ex_description)
            {
                ...
                do
                {
                    ...
                    {
                        log("SYSERR: Object #%d (%s) is type NOTE and has extra description with same name. (%s)", GET_OBJ_VNUM(obj), obj->short_description, obj->name);
                    }
...
static int check_object_spell_number(struct obj_data *obj, int val)
{
    ...
    if(error)
    {
        log("SYSERR: Object #%d (%s) has out of range spell #%d.", GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_VAL(obj, val));
    }
    ...
    {
        log("SYSERR: Object #%d (%s) uses '%s' spell #%d.", GET_OBJ_VNUM(obj), obj->short_description, spellname, GET_OBJ_VAL(obj, val));
    }
...
static int check_object_level(struct obj_data *obj, int val)
{
    ...
    {
        log("SYSERR: Object #%d (%s) has out of range level #%d.", GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_VAL(obj, val));
    }
