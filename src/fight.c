...
void check_killer(struct char_data *ch, struct char_data *vict)
{
    ...
    mudlog(BRF, MAX(LVL_IMMORT, MAX(GET_INVIS_LEV(ch), GET_INVIS_LEV(vict))), TRUE, "PC Killer bit set on %s for initiating attack on %s at %s.", GET_NAME(ch), GET_NAME(vict), world[IN_ROOM(vict)].name);
}
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
    ...
    if(GET_POS(victim) == POS_DEAD)
    {
        ...
        if(!IS_NPC(victim))
        {
            mudlog(BRF, MAX(LVL_IMMORT, MAX(GET_INVIS_LEV(ch), GET_INVIS_LEV(victim))), TRUE, "%s killed by %s at %s", GET_NAME(victim), GET_NAME(ch), world[IN_ROOM(victim)].name);
