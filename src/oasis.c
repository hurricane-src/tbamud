...
void cleanup_olc(struct descriptor_data *d, byte cleanup_type)
{
    ...
    if(d->character)
    {
        ...
        {
            mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s stops editing the game configuration", GET_NAME(d->character));
        }
        else if(STATE(d) == CON_TEDIT)
        {
            mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s stops editing text files.", GET_NAME(d->character));
        }
        else if(STATE(d) == CON_HEDIT)
        {
            mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s stops editing help files.", GET_NAME(d->character));
        }
        else
        {
            mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s stops editing zone %d allowed zone %d", GET_NAME(d->character), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(d->character));
        }
