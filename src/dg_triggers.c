    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room)
    {
        ...
        for(mem = SCRIPT_MEM(ch); mem && SCRIPT_MEM(ch); mem = mem->next)
        {
            ...
            if(mem && !command_performed)
            {
                for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
                {
                    if(IS_SET(GET_TRIG_TYPE(t), MTRIG_MEMORY) && CAN_SEE(ch, actor) && !GET_TRIG_DEPTH(t) && rand_number(1, 100) <= GET_TRIG_NARG(t))
                    {
                        ADD_UID_VAR(buf, t, char_script_id(actor), "actor", 0);
                        script_driver(&ch, t, MOB_TRIGGER, TRIG_NEW);
                        break;
...
int greet_mtrigger(char_data *actor, int dir)
{
    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room)
    {
        ...
        {
            next_trig = t->next;
            if(((IS_SET(GET_TRIG_TYPE(t), MTRIG_GREET) && CAN_SEE(ch, actor)) || IS_SET(GET_TRIG_TYPE(t), MTRIG_GREET_ALL)) && !GET_TRIG_DEPTH(t) && (rand_number(1, 100) <= GET_TRIG_NARG(t)))
            {
...
int command_mtrigger(char_data *actor, char *cmd, char *argument)
{
    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
    {
        ...
        if(SCRIPT_CHECK(ch, MTRIG_COMMAND) && !AFF_FLAGGED(ch, AFF_CHARM) && ((actor != ch) || CONFIG_SCRIPT_PLAYERS))
        {
            for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
            {
                ...
                {
                    mudlog(NRM, LVL_BUILDER, TRUE, "SYSERR: Command Trigger #%d has no text argument!", GET_TRIG_VNUM(t));
                    continue;
                }
...
void speech_mtrigger(char_data *actor, char *str)
{
    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
    {
        ...
        if(SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) && !AFF_FLAGGED(ch, AFF_CHARM) && ((actor != ch) || CONFIG_SCRIPT_PLAYERS))
        {
            for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
            {
                ...
                if(!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t))
                {
                    mudlog(NRM, LVL_BUILDER, TRUE, "SYSERR: Speech Trigger #%d has no text argument!", GET_TRIG_VNUM(t));
                    continue;
                }
...
void hitprcnt_mtrigger(char_data *ch)
{
    ...
    for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
    {
        if(TRIGGER_CHECK(t, MTRIG_HITPRCNT) && GET_MAX_HIT(ch) && (((GET_HIT(ch) * 100) / GET_MAX_HIT(ch)) <= GET_TRIG_NARG(t)))
        {
...
int leave_mtrigger(char_data *actor, int dir)
{
    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room)
    {
        ...
        for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
        {
            if((IS_SET(GET_TRIG_TYPE(t), MTRIG_LEAVE) && CAN_SEE(ch, actor)) && !GET_TRIG_DEPTH(t) && (rand_number(1, 100) <= GET_TRIG_NARG(t)))
            {
                if(dir >= 0 && dir < DIR_COUNT)
                {
...
int door_mtrigger(char_data *actor, int subcmd, int dir)
{
    ...
    for(ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room)
    {
        ...
        for(t = TRIGGERS(SCRIPT(ch)); t; t = t->next)
        {
            if(IS_SET(GET_TRIG_TYPE(t), MTRIG_DOOR) && CAN_SEE(ch, actor) && !GET_TRIG_DEPTH(t) && (rand_number(1, 100) <= GET_TRIG_NARG(t)))
            {
