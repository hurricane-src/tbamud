...
void run_autowiz(void)
{
    ...
    if(CONFIG_USE_AUTOWIZ)
    {
        ...
        res = snprintf(buf, sizeof(buf), "nice ../bin/autowiz %d %s %d %s %d &", CONFIG_MIN_WIZLIST_LEV, WIZLIST_FILE, LVL_IMMORT, IMMLIST_FILE, (int)getpid());
#elif defined(CIRCLE_WINDOWS)
        res = snprintf(buf, sizeof(buf), "autowiz %d %s %d %s", CONFIG_MIN_WIZLIST_LEV, WIZLIST_FILE, LVL_IMMORT, IMMLIST_FILE);
...
void gain_exp(struct char_data *ch, int gain)
{
    ...
    if(gain > 0)
    {
        ...
        GET_EXP(ch) += gain;
        while(GET_LEVEL(ch) < LVL_IMMORT - CONFIG_NO_MORT_TO_IMMORT && GET_EXP(ch) >= level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1))
        {
            GET_LEVEL(ch) += 1;
            num_levels++;
            advance_level(ch);
