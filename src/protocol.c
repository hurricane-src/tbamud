...
static void ReportBug(const char *apText) { log("%s", apText); }
    ...
    switch(aCmd)
    {
        case(char)TELOPT_TTYPE:
            if(pProtocol->bTTYPE)
            {
                ...
                else if(MatchString(pClientName, "MUSHCLIENT") || MatchString(pClientName, "CMUD") || MatchString(pClientName, "ATLANTIS") || MatchString(pClientName, "KILDCLIENT") || MatchString(pClientName, "TINTIN++") ||
